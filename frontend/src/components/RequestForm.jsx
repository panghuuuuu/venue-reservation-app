...
const RequestForm = () => {
  ...
  const submitRequest = async (e) => {
    ...
    const response = await fetch(`http://localhost:9999/request/reservation`, {
      ...
      .catch((error) => {
        ...
        setMessage(
          "Please answer all required fields or verify the venue schedule."
        );
      });
  ...
  return (
    <section className="reserve-venue">
      ...
      <p className="body">
        Fill out the form with the reservation details.
        <br />
        You are currently reserving the venue {venue.venueName}.
      </p>
      ...
      <form className="reservation-request-form" id="reservation-request-form">
        ...
        <div className="reservation-conflicts">
          <p className="body">
            <em>
              Check for any conflicting reservations first before proceeding.
            </em>
          </p>
          ...
          <p
            className="body"
            id="conflicts-message"
            style={{ display: "none" }}
          >
            There are {`${reservationConflicts.length}`} conflicting
            reservations with this schedule.
          </p>
          <table
            className="conflicts-table"
            id="conflicts-table"
            style={{ display: "none" }}
          >
            <tbody>
              {reservationConflicts?.map((reservation) => (
                <tr key={reservation.reservationID}>
                  <td>
                    <span className="reservation-purpose">
                      {reservation.purpose}
                    </span>
